...
describe("Financial Report", () => {
  ...
  it("Tracks total sales revenue", () => {
    jest
      .mocked(order1.getAllItems)
      .mockReturnValueOnce(new Map<Item, number>());
    jest.mocked(order1.getTotal).mockReturnValueOnce(10);
    jest
      .mocked(order2.getAllItems)
      .mockReturnValueOnce(new Map<Item, number>());
  ...
  it("Tracks sales revenue of each item", () => {
    ...
    report.updateSalesRevenue(order1);
    expect(
      report.generateFullReport().find((row) => row.includes("first item")),
    ).toEqual(["first item", "£20"]);
    expect(
      report.generateFullReport().find((row) => row.includes("second item")),
    ).toEqual(["second item", "£15"]);
    ...
    expect(
      report.generateFullReport().find((row) => row.includes("first item")),
    ).toEqual(["first item", "£24"]);
    expect(
      report.generateFullReport().find((row) => row.includes("second item")),
    ).toEqual(["second item", "£40"]);
  });
  ...
  it("Tracks purchase costs of each item", () => {
    ...
    expect(
      report.generateFullReport().find((row) => row.includes("first item")),
    ).toEqual(["first item", "£9"]);
    expect(
      report.generateFullReport().find((row) => row.includes("second item")),
    ).toEqual(["second item", "£18"]);
    ...
    expect(
      report.generateFullReport().find((row) => row.includes("first item")),
    ).toEqual(["first item", "£27"]);
    expect(
      report.generateFullReport().find((row) => row.includes("second item")),
    ).toEqual(["second item", "£24"]);
  });
